...
def get_tweets():
	...
	if search_term in CACHE_DICTION:
		...
		tweet_results = CACHE_DICTION[search_term]

	else:
		print('Processing API Request')
		tweet_results = api.user_timeline(search_term)
		CACHE_DICTION[search_term] = tweet_results
...
cur.execute('CREATE TABLE Tweets (tweet_id TEXT, author TEXT, time_posted TIMESTAMP, tweet_text TEXT, retweets INT)')
